...
/* Macros used in Port.c File such as CLEAR_BIT*/
...
/* Board dependant in AEM Slides 180 181*/
/* for example CAN0 and CAN1-->8(as value in CTL_REG) , CAN0 can be also 3*/
typedef enum
{
	PORT_PIN_MODE_ADC,
	PORT_PIN_MODE_CAN,  
 ...
 *      7. pin direction is changeable or not (true, false)
 *      8. pin mode is changeable or not (true, false)
 */
typedef struct
{
    uint8 port_num;						
    ...
    uint8 pin_dir_is_changeable;			
    uint8 pin_mode_is_changeable;			
}Port_ConfigPin;

/* Array of Port_ConfigPin */
/* for example Pin[0](PA0) has 8 member to configure and so on until Pin[43] */
typedef struct Port_ConfigType
{
	Port_ConfigPin Pin[PORT_CONFIGURED_PINS];  /*Array of 43 elements (pins) each pin has 8 members (port_num,pin_num,direction,...)*/
}Port_ConfigType;
...
/* extern varable (strcuture) for Configuration of each pin (the large stucture in Port_PBcfg.h) */
