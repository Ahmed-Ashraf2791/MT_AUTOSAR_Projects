...
void Port_Init(const Port_ConfigType* ConfigPtr)
{
	...
	Port_ConfigPtr = ConfigPtr;
        ...
        /* Loops on each pin (with index from 0 to 42) to configure all 43 pins*/
...
void Port_SetPinDirection(Port_PinType Pin, Port_PinDirectionType Direction)
{
		...
		if(Port_ConfigPtr->Pin[Pin].pin_dir_is_changeable == STD_OFF)
		{
	...
	for(Port_PinType index = 0; index < PORT_CONFIGURED_PINS; index++)
		{
			...
			if (Port_ConfigPtr->Pin[index].pin_dir_is_changeable == STD_OFF)
			{
				if(Port_ConfigPtr->Pin[index].direction == PORT_PIN_OUT)
				{
					/* Set the corresponding bit in the GPIODIR register to configure it as output pin */
...
void Port_SetPinMode(Port_PinType Pin, Port_PinModeType Mode)
{
		...
		if(Port_ConfigPtr->Pin[Pin].pin_mode_is_changeable == STD_OFF)
		{
